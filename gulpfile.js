...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/java-file-format-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/java-file-format-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '') {
                ...
                if (fs.existsSync('../java-file-format-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/java-file-format-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../java-file-format-docs/` + changedFileName, `../../../gitlabRepo/java-file-format-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/java-file-format-docs/')) {
                            shelljs.cp('-rf', `../java-file-format-docs/` + changedFileName, `../../../gitlabRepo/java-file-format-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/java-file-format-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/java-file-format-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/java-file-format-docs`);
